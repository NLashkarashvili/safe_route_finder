"""
Script to plot shortest and safest routes between two points with fixed file saving
"""
# Set matplotlib backend before any other imports
import matplotlib
matplotlib.use('Agg')  # Use the Agg backend for better file output
...
def plot_routes(start_lat, start_lon, end_lat, end_lon, radius=800, output_file=None):
    """
    ...
    """
    print(f"Plotting routes from ({start_lat}, {start_lon}) to ({end_lat}, {end_lon})")
    ...
    if not safety_pois.empty:
        ...
        # Define POI categories and their properties
        poi_categories = {
            'police': {'color': 'blue', 'marker': '^', 'label': 'Police', 'zorder': 4},
            'hospital': {'color': 'blue', 'marker': '+', 'label': 'Hospital', 'zorder': 4},
            'bar': {'color': 'orange', 'marker': 's', 'label': 'Bar/Pub', 'zorder': 4},
            'pub': {'color': 'orange', 'marker': 's', 'label': 'Bar/Pub', 'zorder': 4},
            'cafe': {'color': 'purple', 'marker': 'o', 'label': 'Cafe', 'zorder': 4},
            'restaurant': {'color': 'purple', 'marker': 'o', 'label': 'Restaurant', 'zorder': 4},
            'park': {'color': 'green', 'marker': 'p', 'label': 'Park', 'zorder': 3},
            'station': {'color': 'red', 'marker': 'D', 'label': 'Station', 'zorder': 4}
        }
        
        # Create a dictionary to track which POI types we've added to the legend
        legend_added = {}
        
        # Process each POI
        for idx, poi in safety_pois.iterrows():
            poi_type = None
            
            # Determine POI type from various tags
            if 'amenity' in poi and poi['amenity'] in poi_categories:
                poi_type = poi['amenity']
            elif 'leisure' in poi and poi['leisure'] == 'park':
                poi_type = 'park'
            elif 'railway' in poi and poi['railway'] == 'station':
                poi_type = 'station'
            elif 'public_transport' in poi and poi['public_transport'] == 'station':
                poi_type = 'station'
            
            # If we have a recognized POI type, plot it
            if poi_type:
                props = poi_categories[poi_type]
                
                # Get the geometry and extract coordinates
                geom = poi.geometry
                if geom.geom_type == 'Point':
                    x, y = geom.x, geom.y
                    
                    # Add to legend only once per type
                    if poi_type not in legend_added:
                        ax.scatter(x, y, color=props['color'], marker=props['marker'], 
                                  s=100, zorder=props['zorder'], label=props['label'])
                        legend_added[poi_type] = True
                    else:
                        ax.scatter(x, y, color=props['color'], marker=props['marker'], 
                                  s=100, zorder=props['zorder'])
    ...
    ax.scatter(start_lon, start_lat, c='red', s=150, zorder=6, marker='*', edgecolor='black', linewidth=1.5, label='Start')
    ax.scatter(end_lon, end_lat, c='darkred', s=150, zorder=6, marker='*', edgecolor='black', linewidth=1.5, label='End')
    
    # Add annotations for start and end
    ax.annotate('Start', xy=(start_lon, start_lat), xytext=(10, 10), 
                textcoords="offset points", fontsize=12, fontweight='bold')
    ax.annotate('End', xy=(end_lon, end_lat), xytext=(10, 10), 
                textcoords="offset points", fontsize=12, fontweight='bold')
    
    # Add legend and title
    ax.legend(loc='best', fontsize=10)
    plt.title(f'Camden Area: Shortest vs. Safest Route\n' +
              f'Shortest: {shortest_length:.0f}m, Safety: {shortest_safety:.1f}/10 | ' +
              f'Safest: {safest_length:.0f}m, Safety: {safest_safety:.1f}/10', 
              fontsize=14)
    
    # Ensure the figure is properly rendered
    plt.tight_layout()
    
    # Save the figure using a more direct approach
    if output_file:
        # Create the canvas and save directly
        ...
        default_output = "camden_routes.png"
        canvas = FigureCanvasAgg(fig)
        canvas.draw()
